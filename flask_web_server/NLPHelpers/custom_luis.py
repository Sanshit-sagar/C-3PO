...
class Entity(_LuisData,
             collections.namedtuple(
                 '_Entity',
                 ['entity', 'type', 'score', 'start_index', 'end_index', 'role', 'resolution'])):
...
class LuisResult(_LuisData,
                 collections.namedtuple(
                     '_LuisResult',
                     ['intents', 'entities', 'query', 'sentimentAnalysis'])):
    ...
    def _from_json(klass, json):
        ...
        print ('Got response from LUIS: %s', json)
        return LuisResult(
            query=json['query'], intents=intents, entities=entities, sentimentAnalysis=json["sentimentAnalysis"])
